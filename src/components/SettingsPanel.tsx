...
export function SettingsPanel() {
  ...
  return (
    <form
      className="bg-white border border-[#c8c4d8] rounded-2xl p-6 space-y-5"
      onSubmit={async (e) => {
        e.preventDefault();
    ...
    >
      ...
      <fieldset disabled={busy} className="space-y-5">
        <MediaField
          ...
          onChange={(headshot) => setDraft((s) => ({ ...s, headshot }))}
          onBusy={onBusy}
        />
        <MediaField
          label="Resume PDF"
          ...
          onChange={(resumeUrl) => setDraft((s) => ({ ...s, resumeUrl }))}
          onBusy={onBusy}
        />
        <p className="text-xs text-[#474555]">
          Upload your actual PDF, link an existing PDF, or use /resume.pdf after adding it to the
          public folder. Leave blank until it is available.
        </p>
        {(['availability', 'bookingsWindow', 'responseWindow'] as const).map((key, index) => (
