...
export function parseRoute(path: string) {
  ...
  if (preview && slugPattern.test(preview[1])) return { kind: 'preview' as const, slug: preview[1] };
  const project = /^\/projects\/([^/]+)\/?$/.exec(path);
  if (project && slugPattern.test(project[1])) return { kind: 'project' as const, slug: project[1] };
