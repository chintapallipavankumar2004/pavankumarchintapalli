...
for(const [order,[id,name]] of [['website','Websites'],['webapp','Web Applications'],['app','Apps'],['logo','Logos'],['poster','Posters'],['automation','Automations']].entries()){await create(`categories/${id}`,{id,name,slug:id,description:'',iconName:'',order,published:true,mediaLayout:name==='Logos'?'logo':name==='Posters'?'gallery':'cover',accent:'indigo',updatedBy:'migration'});}
