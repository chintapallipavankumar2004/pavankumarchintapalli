...
async function create(path:string,data:Record<string,unknown>){const ref=db.doc(path);if(!(await ref.get()).exists)batch.create(ref,{...data,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp(),schemaVersion:data.schemaVersion??1});}
...
for(const category of Object.values(DEFAULT_PROJECT_CATEGORIES))await create(`categories/${category.id}`,{...category,updatedBy:'migration'});
