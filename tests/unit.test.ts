...
const valid = { fullName: 'Test Visitor', email: 'test@example.com', phone: '', service: 'website', budget: 'Not specified', description: 'Please help create a portfolio website for my work.' };
...
async function invoke(handler: Handler, body: unknown, headers: Record<string, string> = {}, method = 'POST') {
  let status = 0, data: any;
  const responseHeaders: Record<string, unknown> = {};
  const req = { method, headers: { origin: 'http://localhost:3000', 'content-type': 'application/json', ...headers }, body, socket: { remoteAddress: '127.0.0.1' } } as unknown as Request;
  const res = { setHeader: (key: string, value: unknown) => { responseHeaders[key] = value; }, set statusCode(value: number) { status = value; }, end: (value: string) => { data = JSON.parse(value); } } as unknown as Response;
...
function fixture(uid = 'owner', tokenValid = true, appCheckValid = true) {
  ...
  const services = () => ({ auth: { verifyIdToken: async (_token: string, revoked: boolean) => { revocationChecked = revoked; if (!tokenValid) throw Error('invalid'); return { uid }; } },
    appCheck: { verifyToken: async () => { if (!appCheckValid) throw Error('invalid'); return {}; } },
    db: { doc: (path: string) => ({ path, get: async () => snapshot(path) }), runTransaction: async (callback: any) => callback({ get: async (ref: any) => snapshot(ref.path), set: (ref: any, value: any) => documents.set(ref.path, value), create: (ref: any, value: any) => documents.set(ref.path, value) }) }
...
test('validation trims input and rejects malformed or oversized enquiries', () => {
  assert.equal(validateEnquiry({ ...valid, fullName: '  Test Visitor  ', email: 'TEST@EXAMPLE.COM' }).email, 'test@example.com');
  for (const data of [{ ...valid, email: 'bad' }, { ...valid, description: 'short' }, { ...valid, description: 'x'.repeat(5001) }, { ...valid, phone: 'javascript:' }, { ...valid, service: 'unknown' }, { ...valid, fullName: '' }, { ...valid, email: {} }]) assert.throws(() => validateEnquiry(data));
});
...
test('signer follows the Cloudinary sorted SHA-256 parameter contract', () => {
  assert.equal(signUpload({ timestamp: 123, overwrite: false, public_id: 'portfolio/image/test' }, 'secret'), createHash('sha256').update('overwrite=false&public_id=portfolio/image/test&timestamp=123secret').digest('hex'));
});
...
test('sign endpoint rejects invalid, revoked and non-admin sessions', async () => {
  assert.equal((await invoke(createSignHandler(fixture('owner', false).services), { kind: 'image' }, { authorization: 'Bearer invalid' })).status, 401);
  assert.equal((await invoke(createSignHandler(fixture('someone-else').services), { kind: 'image' }, { authorization: 'Bearer user' })).status, 403);
});
test('sign endpoint signs only fixed parameters and never returns the secret', async () => {
  ...
  assert.equal(result.status, 200); assert.equal(f.checked(), true);
  ...
  assert.equal((await invoke(handler, { kind: 'image', public_id: 'other/file' }, { authorization: 'Bearer owner' })).status, 400);
  const pdf = await invoke(handler, { kind: 'resume' }, { authorization: 'Bearer owner' });
  assert.equal(pdf.data.params.allowed_formats, 'pdf'); assert.equal(pdf.data.params.upload_preset, 'portfolio-resume');
});
test('enquiry endpoint requires verification and valid input before storing', async () => {
  const f = fixture(); const handler = createEnquiryHandler(f.services);
  assert.equal((await invoke(handler, { ...valid, id })).status, 403);
  assert.equal((await invoke(handler, { ...valid, id, email: 'invalid' }, { 'x-firebase-appcheck': 'token' })).status, 400);
  assert.equal((await invoke(handler, { ...valid, id, website: 'spam' }, { 'x-firebase-appcheck': 'token' })).status, 400);
  assert.equal((await invoke(createEnquiryHandler(fixture('owner', true, false).services), { ...valid, id }, { 'x-firebase-appcheck': 'invalid' })).status, 403);
...
test('enquiries persist once, strip privileged fields, and do not claim email delivery', async () => {
  ...
  assert.equal(first.status, 201); assert.deepEqual(first.data, { saved: true, id });
  ...
  assert.equal((await invoke(handler, { ...body, description: valid.description + ' changed' }, { 'x-firebase-appcheck': 'valid' })).status, 409);
});
test('enquiry endpoint rate limits after five accepted submissions per IP per hour', async () => {
  const f = fixture(); const handler = createEnquiryHandler(f.services);
  for (let i = 1; i <= 6; i++) {
    const result = await invoke(handler, { ...valid, id: `00000000-0000-4000-8000-${String(i).padStart(12, '0')}` }, { 'x-firebase-appcheck': 'token' });
    assert.equal(result.status, i <= 5 ? 201 : 429);
  }
  assert.equal([...f.documents.keys()].some(key => key.includes('127.0.0.1')), false);
});
test('backend errors fail closed without leaking internal errors', async () => {
  const result = await invoke(createSignHandler(() => { throw Error('private-secret'); }), { kind: 'image' }, { authorization: 'Bearer token' });
  assert.equal(result.status, 503); assert.equal(JSON.stringify(result).includes('private-secret'), false);
});
